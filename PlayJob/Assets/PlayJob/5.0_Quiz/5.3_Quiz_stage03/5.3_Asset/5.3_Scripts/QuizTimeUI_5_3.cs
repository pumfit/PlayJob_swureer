...
public class QuizTimeUI_5_3 : MonoBehaviour
{
    ...
    void Start() //각 퀴즈 스테이지 마다 해당 스크립트가 들어가야함
    {
        ...
        filePath = Application.persistentDataPath + "/ConnectQuizDB_210214.txt";
        Save();
        Load();

        if (SceneManager.GetActiveScene().name == "QuizScene_5.3_Connect_1")
        {
            tempQuiz = Allquiz.FindAll(x => int.Parse(x.Level) == 1);
        }
        else if (SceneManager.GetActiveScene().name == "QuizScene_5.3_Connect_2")
        {
            tempQuiz = Allquiz.FindAll(x => int.Parse(x.Level) == 2);
        }
        else if (SceneManager.GetActiveScene().name == "QuizScene_5.3_Connect_3")
        {
            tempQuiz = Allquiz.FindAll(x => int.Parse(x.Level) == 3);
        }

        curQuiz = tempQuiz[Random.Range(0,3)];//0,1,2 총 3개중 랜덤
    ...
    public void OnClicked(Button button)
    {
        ...
        {//맞춘경우
            GameObject.Find("Canvas").transform.Find("EndPanel").gameObject.SetActive(true);
            if (PlayerPrefs.HasKey("Stage_2_2_Clear") || !(SceneManager.GetActiveScene().name == "QuizScene_5.3_Connect_3"))
            {
