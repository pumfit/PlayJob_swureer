...
public class QuizTimeUI_5_2 : MonoBehaviour
{
    [Header("전체퀴즈 DB 및 해당 스테이지 List")]
    public TextAsset QuizDatabase;
    public List<Quiz> Allquiz;//private 로 변경할 것임
    private List<Quiz> tempQuiz;
    [Header("실행시 현재 퀴즈 내용")]
    public Quiz curQuiz;
    ...
    void Start() //각 퀴즈 스테이지 마다 해당 스크립트가 들어가야함
    {
        ...
        filePath = Application.persistentDataPath + "/DesignQuizDB_210209.txt";
        Save();
        Load();

        if (SceneManager.GetActiveScene().name == "QuizScene_5.2_Design_1")
        {
            tempQuiz = Allquiz.FindAll(x => int.Parse(x.Level) == 1);
        }
        else if (SceneManager.GetActiveScene().name == "QuizScene_5.2_Design_2")
        {
            tempQuiz = Allquiz.FindAll(x => int.Parse(x.Level) == 2);
        }
        else if (SceneManager.GetActiveScene().name == "QuizScene_5.2_Design_3")
        {
    ...
    public void OnClicked(Button button)
    {
        ...
        {//맞춘경우
            GameObject.Find("Canvas").transform.Find("EndPanel").gameObject.SetActive(true);
            if (PlayerPrefs.HasKey("Stage_1_2_Clear") || !(SceneManager.GetActiveScene().name == "QuizScene_5.2_Design_3"))
            {
